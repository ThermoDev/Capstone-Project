...
const mapLabelToColors= (labels) => {
  ...
  return labels.map( (l, index) =>{
    return 'rgb('+ 255*rgb*index +','+ (206+(255-206)*rgb*index)+','+(209+(255-209)*rgb*index)+')';
  })
}
...
const PortfolioItem = props => {
  ...
  return (
    <StyledPaper>{data.map( item => (
      <ExpansionPanel expanded={expanded === item.portfolio_id} key={item.portfolio_id}>
        ...
        <StyledExpansionPanelDetails>
          <StyledDiv>
            <StyledSubDiv>
              <Typography variant="h5" color="primary">Portfolio value:</Typography> 
                ...
                <Doughnut
                  data={{
                    ...
                    datasets: [{
                      ...
                      backgroundColor: mapLabelToColors(Object.keys(item.stock_weightings))
                    }]
                  }}
                />
            </StyledSubDiv>
